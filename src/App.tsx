...
function App() {
  ...
  // Restore last selected document once after initial load
  useEffect(() => {
    if (documents.length > 0 && !hasRestoredSelection.current) {
  ...
  // Persist selected document whenever it changes
  useEffect(() => {
    if (selectedDocId) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const result = await response.json();
        // Backend already filters by documentId via Pinecone metadata filter
        const relevantChunks = result.results
                              .map((doc: any) => doc.pageContent)
                              .slice(0, 5)
                              .join("\n\n");

        console.log(`Retrieved ${result.results.length} raw chunks, mapped to ${relevantChunks.length} chars`);
        ...
        await memory.saveContext({ human: input }, { ai: fullContent }); // Save the interaction
